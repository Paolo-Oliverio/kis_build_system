...
function(kis_handle_third_party_dependencies _package_name _tpl_deps_json)
    ...
    foreach(i RANGE ${last_idx})
        ...
        # --- CONDITION HANDLING (Explicit + Implicit) ---
        set(is_active TRUE)
        
        # 1. Check explicit condition from manifest
        ...
        if(NOT err_cond AND DEFINED dep_cond AND NOT "${dep_cond}" STREQUAL "")
            if(NOT (DEFINED ${dep_cond} AND ${dep_cond}))
                kis_message_verbose("  -> Skipping dependency '${dep_name}' for '${_package_name}' (explicit condition '${dep_cond}' is OFF)")
                set(is_active FALSE)
            endif()
        endif()

        # 2. Apply implicit condition based on scope
        if(is_active)
            ...
            if(NOT scope_err)
                ...
                # If the scope is *exclusively* for a component, apply its build flag
                ...
                if(num_scopes_val EQUAL 1)
                    ...
                    if(single_scope STREQUAL "tests" AND NOT KIS_BUILD_TESTS)
                        set(is_active FALSE)
                        kis_message_verbose("  -> Skipping dependency '${dep_name}' (implicitly disabled by scope=tests and KIS_BUILD_TESTS=OFF)")
                    elseif(single_scope STREQUAL "samples" AND NOT KIS_BUILD_SAMPLES)
                        set(is_active FALSE)
                        kis_message_verbose("  -> Skipping dependency '${dep_name}' (implicitly disabled by scope=samples and KIS_BUILD_SAMPLES=OFF)")
                    elseif(single_scope STREQUAL "benchmarks" AND NOT KIS_BUILD_BENCHMARKS)
                        set(is_active FALSE)
                        kis_message_verbose("  -> Skipping dependency '${dep_name}' (implicitly disabled by scope=benchmarks and KIS_BUILD_BENCHMARKS=OFF)")
                    endif()
                endif()
            endif()
        endif()
        ...
        if(existing_dep_entry)
            ...
            list(GET existing_parts 5 existing_pkg)
...
function(kis_populate_declared_dependencies)
    ...
    if(should_build_third_party)
        foreach(dep_entry ${declared_deps})
            ...
            list(GET dep_parts 6 options_json)
    ...
    if(should_build_third_party)
        ...
        # Logic for reusing pre-built dependencies from a base variant
