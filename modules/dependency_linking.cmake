...
function(kis_link_from_manifest)
    ...
    # Debug: Check what variables are actually defined
    if(DEFINED MANIFEST_KIS_DEPENDENCIES)
        kis_message_verbose("  [${ARG_TARGET}] MANIFEST_KIS_DEPENDENCIES is DEFINED: '${MANIFEST_KIS_DEPENDENCIES}'")
    else()
        kis_message_verbose("  [${ARG_TARGET}] MANIFEST_KIS_DEPENDENCIES is NOT defined in function scope")
    endif()
    if(DEFINED MANIFEST_TPL_DEPENDENCIES)
        kis_message_verbose("  [${ARG_TARGET}] MANIFEST_TPL_DEPENDENCIES is DEFINED: '${MANIFEST_TPL_DEPENDENCIES}'")
    else()
        kis_message_verbose("  [${ARG_TARGET}] MANIFEST_TPL_DEPENDENCIES is NOT defined in function scope")
    endif()
    
    # Capture manifest variables from parent scope into local scope
    # (kis_read_package_manifest_json sets them in PARENT_SCOPE)
    if(DEFINED MANIFEST_KIS_DEPENDENCIES)
        set(local_kis_deps "${MANIFEST_KIS_DEPENDENCIES}")
    endif()
    if(DEFINED MANIFEST_TPL_DEPENDENCIES)
        set(local_tpl_deps "${MANIFEST_TPL_DEPENDENCIES}")
    endif()
    ...
    macro(_process_dependency_list dep_list _should_process_var)
        set(${_should_process_var} TRUE)
        string(JSON num_deps ERROR_VARIABLE err LENGTH "${dep_list}")
        if(err OR num_deps EQUAL 0) 
            set(${_should_process_var} FALSE)
        else()
            ...
            # --- SCOPE CHECK ---
            string(JSON dep_name_for_debug GET "${dep_obj}" "name")
            ...
            if(scope_err)
                ...
                kis_message_verbose("    [SCOPE] Dependency '${dep_name_for_debug}' has no scope field - including for all scopes (current: ${ARG_SCOPE})")
                ...
                if("all" IN_LIST dep_scopes OR ARG_SCOPE IN_LIST dep_scopes)
                    kis_message_verbose("    [SCOPE] Dependency '${dep_name_for_debug}' scope [${dep_scopes}] matches current scope '${ARG_SCOPE}' - including")
                    set(is_in_scope TRUE)
                else()
                    kis_message_verbose("    [SCOPE] Dependency '${dep_name_for_debug}' scope [${dep_scopes}] does NOT match current scope '${ARG_SCOPE}' - skipping")
                endif()
            endif()
            if(NOT is_in_scope)
    ...
    kis_message_verbose("  [${ARG_TARGET}] Checking manifest dependencies for scope '${ARG_SCOPE}'")
    if(DEFINED local_kis_deps)
        kis_message_verbose("  [${ARG_TARGET}] Processing KIS dependencies...")
        set(processed_kis FALSE)
        _process_dependency_list("${local_kis_deps}" processed_kis) 
    else()
        kis_message_verbose("  [${ARG_TARGET}] No KIS dependencies in manifest")
    endif()
    if(DEFINED local_tpl_deps)
        kis_message_verbose("  [${ARG_TARGET}] Processing TPL dependencies...")
        set(processed_tpl FALSE)
        _process_dependency_list("${local_tpl_deps}" processed_tpl) 
    else()
        kis_message_verbose("  [${ARG_TARGET}] No TPL dependencies in manifest")
    endif()
    if(public_deps OR private_deps OR interface_deps)
        kis_message_verbose("  [${ARG_TARGET}] Found dependencies for scope '${ARG_SCOPE}': PUBLIC=[${public_deps}] PRIVATE=[${private_deps}] INTERFACE=[${interface_deps}]")
        ...
        foreach(scope ${scopes})
            ...
            foreach(item ${link_data})
                if(item STREQUAL "PUBLIC" OR item STREQUAL "PRIVATE" OR item STREQUAL "INTERFACE")
                    ...
                    # --- Skip self-links ---
                    # If a package declares itself (possibly via an alias) as a dependency
                    # we must ignore it to avoid CMake errors like "Target X links to itself.".
                    if(final_link_target STREQUAL "${package_name}" OR final_link_target STREQUAL "kis::${package_name}")
                        kis_message_verbose("  [${package_name}] Detected self-link to '${final_link_target}' - skipping to avoid self-linking")
                        continue()
                    endif()
                    ...
                    if(target_type STREQUAL "INTERFACE_LIBRARY" AND NOT final_visibility STREQUAL "INTERFACE")
                        # --- THIS IS THE FIX ---
                        # Replaced the placeholder "..." with a complete, helpful warning message.
                        kis_message_warning_actionable(
                            "Linking Mismatch"
                            "Package '${package_name}' is an INTERFACE library but links '${final_link_target}' as ${current_visibility}. Forcing to INTERFACE."
                            "Update the 'link' property to 'INTERFACE' for this dependency in the kis.package.json file."
                        )
                    ...
                    kis_message_verbose("  [${package_name}] Storing final link command for scope '${scope}': ${final_visibility} ${final_link_target}")
                endif()
            endforeach()
        endforeach()
    endforeach()
